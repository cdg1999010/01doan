...
(function($, window, document, name) {
	...
	var togglePopover = function(popover, anchor, state) {
		...
		removeBackdropTimer && removeBackdropTimer.cancel(); //Hủy bỏremove的timer
...
(function($, window, document) {
	...
	window.addEventListener($.EVENT_END, function(event) { //使用touchend来Hủy bỏ高亮，避免一次点击既不触发tap，doubletap，longtap的事件
		if (!cell) {
			return;
		}
		toggleActive(false);
	...
	window.addEventListener($.EVENT_CANCEL, function(event) { //使用touchcancel来Hủy bỏ高亮，避免一次点击既不触发tap，doubletap，longtap的事件
		if (!cell) {
			return;
		}
		toggleActive(false);
...
(function($, window, document) {
    ...
    var createAlert = function(message, title, btnValue, callback, type) {
        ...
        if (!$.os.plus || type === 'div') {
            return createPopup(createInner(message, title || '提示') + createButtons([btnValue || 'Đảm bảo']), callback);
        }
        return plus.nativeUI.alert(message, callback, title || '提示', btnValue || 'Đảm bảo');
    };
    var createConfirm = function(message, title, btnArray, callback, type) {
        ...
        if (!$.os.plus || type === 'div') {
            return createPopup(createInner(message, title || '提示') + createButtons(btnArray || ['Hủy bỏ', '确认']), callback);
        }
        return plus.nativeUI.confirm(message, callback, title, btnArray || ['Hủy bỏ', '确认']);
    };
    var createPrompt = function(message, placeholder, title, btnArray, callback, type) {
        ...
        if (!$.os.plus || type === 'div') {
            return createPopup(createInner(message, title || '提示', createInput(placeholder)) + createButtons(btnArray || ['Hủy bỏ', '确认']), callback);
        }
        return plus.nativeUI.prompt(message, callback, title || '提示', placeholder, btnArray || ['Hủy bỏ', '确认']);
    };
